...
const CourseDetailPage = () => {
  const { courseId } = useParams({ from: '/_protected/tsx/courses/$courseId' } );
  const { useCourseDetail, enrollInCourse, enrollState } = useCourses();
  const { data: course, isLoading, error } = useCourseDetail(courseId);

  return (
    <div className="min-h-screen bg-linear-to-b from-[#050530] via-[#040646] to-[#020038] px-4 py-8 md:py-16">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        {isLoading ? (
          ...
          <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 text-center">
            ...
            <Link to="/tsx/courses" className="text-amber-400 hover:text-amber-300 mt-4 inline-block">
              العودة للدورات
            </Link>
          </div>
        ) : !course ? (
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-10 shadow-2xl shadow-black/30">
            ...
            <div className="mt-8 border-t border-white/10 pt-6">
              <Link to="/" className="inline-flex items-center gap-2 text-white/60 hover:text-amber-400 transition-colors duration-300 text-sm">

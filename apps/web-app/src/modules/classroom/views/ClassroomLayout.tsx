...
import { BookOpen, CheckSquare, Bell, MessageCircle, LayoutDashboard, Menu, X } from 'lucide-react';
// import { useAuthStore } from '../../auth/store/authStore'; // غير ضروري حاليًا
...
export const ClassroomLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#020617] text-slate-200" dir="rtl">
      <button className="lg:hidden fixed top-4 right-4 z-50 p-2 bg-slate-800 rounded-lg text-white" onClick={() => setSidebarOpen(true)}>
        <Menu size={24} />
      </button>
      {sidebarOpen && <div className="lg:hidden fixed inset-0 z-40 bg-black/50" onClick={() => setSidebarOpen(false)} />}
      <aside className={`fixed lg:static inset-y-0 right-0 z-50 w-64 border-l border-white/10 bg-[#040646]/50 backdrop-blur-xl p-6 flex flex-col transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="flex flex-col gap-2">
          {sidebarItems.map((item) => (
            <Link key={item.key} to={item.to} params={{ classroomId: classroomId! }} onClick={() => setSidebarOpen(false)} className="group flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 text-slate-400 hover:bg-white/5 hover:text-white" activeProps={{ className: 'bg-gradient-to-r from-amber-500/20 to-transparent text-amber-400 border-r-2 border-amber-500' }}>

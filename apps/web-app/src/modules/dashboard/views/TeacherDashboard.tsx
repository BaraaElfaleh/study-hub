...
const TeacherDashboard = () => {
  ...
  return (
    <>
      ...
      {/* كورساتي */}
      <div className="mb-10">
        <h2 className="text-2xl font-bold text-white mb-4">كورسات تدرسها</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {teachingCourses.map((course) => (
            <div key={course.id} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-all">
              ...
              <p className="text-white/60 text-sm mt-1">المستوى: {course.level}</p>
              <Link to="/tsx/classroom/$classroomId" params={{ classroomId: course.id } as any} className="text-amber-400 text-sm mt-3 inline-block">
                إدارة الفصل ←
              </Link>
            </div>
          ))}
      ...
      {/* آخر المهام */}
      <div className="mb-10">
        <h2 className="text-2xl font-bold text-white mb-4">آخر المهام المعلقة</h2>
      ...
      {/* إعلاناتك */}
      <div>
        <h2 className="text-2xl font-bold text-white mb-4">إعلاناتك الأخيرة</h2>
        <div className="space-y-3">
